...
test.describe("Checkout challenge", async () => {
  ...
  test("buy now pay later", async ({ page, headless }) => {
    ...
    await page.getByTestId("postal_code").fill("0101");
    await page.getByTestId("house_number").fill("42");
    await page.getByTestId("street").fill("Connel Quadrant");
    await page.getByTestId("city").fill("Melbourne");
    await page.getByTestId("state").fill("South Australia");
